...
def main():
    ...
    rank = 16
    try:
        rankName = list(ranks.items())[rank]
    except IndexError:
        rankName = None
    ...
    #     items = {}
    #     for translation in data.get("localizedTexts", []):
    #         if translation.get("sid", "").startswith("ID_ARRIVAL_RANKED_BR_DIVISIONS"):
    #             items[translation.get("sid", "")] = translation.get("localizedText", "")

    # with open("temp.json", "w", encoding="utf-8") as f:
    #     json.dump(dict(sorted(items.items())), f, ensure_ascii=False, indent=4)
